...
export default function Library() {
  ...
  const fetchItems = useCallback(async () => {
    if (activeTab === "Element") {
      ...
      try {
        ...
        let mapped = (data || []).map(mapElementToItem);
        if (typeFilter !== "all") mapped = mapped.filter((i) => i.type === typeFilter);   // ← THÊM
        if (favoritesOnly) mapped = mapped.filter((i) => i.favorite);
        setItems(mapped);
      } catch (err) {
  ...
  const toggleFavorite = async (id) => {
    ...
    setItems((prev) => prev.map((item) => (item.id === id ? { ...item, favorite: next } : item)));
    try {
      if (target.category === "element") {              // ← THÊM branch
