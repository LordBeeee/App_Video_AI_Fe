...
export default function AssetPickerModal({ open, onClose, onSelect }) {
  ...
  useEffect(() => {
    if (!open) return
    setLoading(true)
    getLibraryAssetsApi({ tab: activeTab, type: "image", favorite: favoritesOnly })
      .then((data) => setItems(data.items || []))
      .catch(() => setItems([]))
      .finally(() => setLoading(false))
  }, [open, activeTab, favoritesOnly])
  ...
  const handleToggleFavorite = async (assetId, e) => {
    ...
    try {
      ...
      // Nếu đang lọc favoritesOnly và vừa bỏ tim → loại khỏi danh sách hiển thị
      ...
      >
        ...
        <div className="flex items-center justify-between border-b border-slate-800 px-6 py-4">
          <h3 className="text-base font-semibold text-white">
            Chọn ảnh {selectedAsset ? "(1/1)" : "(0/1)"}
          </h3>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center gap-2 py-16 text-sm text-slate-400">
              ...
              {favoritesOnly
                ...
                : activeTab === "upload"
                ? "Chưa có ảnh nào trong thư viện. Vào Thư Viện → tab Upload để tải ảnh lên."
                : "Chưa có ảnh sáng tạo nào."}
            </div>
          ) : (
              ...
              {items.map((asset) => {
                ...
                return (
                  ...
                  >
                    ...
                    {isSelected && (
                      <div className="absolute right-1.5 top-1.5 flex h-5 w-5 items-center justify-center rounded-full bg-indigo-500 text-white">
